...
//-----------步进电极端口控制
//-----------------------冲洗杆----p51---p54
#define 		StepMotor_B_IO		P5OUT					
#define 		StepMotorBStop		0xe1
//------------------------烘干杆---P62---P65
#define 		StepMotor_C_IO		P6OUT					
#define 		StepMotorCStop		0xC3					
//------------------------分水阀---P83,P84,P86,P87
#define 		StepMotor_A_IO		P8OUT					
...
extern  MotorA_uion MotorA;

typedef struct{
		uint_16 		Index;				//--------步进电极实际位置偏移量
		uint_16  	SumCountSet;		//--------步进电极设置目标位置
		uint_8 	XiangWei;				//--------步进电极相位位置
		uint_8 	State;				//-------ю羽想q铄作步昼m
		uint_8 	DelayTime;			//--------延时时间
		uint_8	enable_f:1;			//--------使能标志	
		uint_8	cw_f:1;				//--------方向标志
		uint_8	end_f:1;				//--------结束标志		
		}MotorB_uion;
extern  MotorB_uion MotorB;
typedef struct{
		uint_16 		Index;				//--------步进电极实际位置偏移量
		uint_16  	SumCountSet;		//--------步进电极设置目标位置
		uint_8 	XiangWei;				//--------步进电极相位位置
