...
void main(void)
{
	...
	while(1)
	{
		if(timer.t_10ms_f == 1)				
		{
			...
			{		
				IR_Opration();   //IR接收处理	
				Cover_Opreation();  //翻盖信号处理
				Cover_send_datadeal();  //发送数据处理
			}			
			Power_Switch_deal();   //开关机处理
			...
			Antifouling_Deal();          //防污处理

			WaterInOutDeal();            //进出水处理 
			
			Water_Temp_Deal(&UART_0);    //即热水温处理
...
//*----------------------------------------------
//函数名称：int_tx0
//功能描述：串口发送中断，串口0,控制即热模块
//输入参数：无
//返回参数：无
//修改记录：暂未定义
//------------------------------------------------*/

#pragma _interrupt INT_TX0
void INT_TX0(void)					  
{
	SC0TICR=SC0TICR&0xfe;
	Uart0_send_data(&UART_0);
}
...
//修改记录：暂未定义
//------------------------------------------------*/
#pragma _interrupt INT_RX0
void INT_RX0(void)					  
{
	Uart0_Reverive(&UART_0);
}
...
{
	IR_receive();				//红外接收
	Cover_receive();			//翻盖数据接收
	Cover_SendData();			//翻盖数据发送
