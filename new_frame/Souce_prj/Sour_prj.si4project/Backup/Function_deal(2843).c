...
void Hip_Wash_Enable(void)
{
	...
	/*	if(Status.Working_f == 1)  //如果已经在清洗状态下，再次按下臀部，则为调节水量
		{	
			Status.Icon.water_press_gear ++;
			if(Status.Icon.water_press_gear > 4)   //如果档位调节大于档位范围，则归0
			{
...
/*		if(Status.Working_f == 1)  //如果已经在清洗状态下，再次按下女性按键，则为调节水量
		{	
			Status.Icon.water_press_gear ++;
			if(Status.Icon.water_press_gear > 4)   //如果档位调节大于档位范围，则归0
			{
