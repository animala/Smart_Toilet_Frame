...
#define	WangFu_2mm		100     //这里是喷杆往复的距离
#define	Reciprocate_speed	50  //这个是往复运动的速度
...
uint_16 Slet_position(uint_8 motor, uint_8 s1, uint_8 s2, uint_8 s3)
{
	...
	//	if(s3 == 2)	//这里是进入清洗功能之前先进入该功能最大水压档位进行清洗之后再转到相应的档位
	//	{
	//		temp = WaterPressure[s1][s2];   //这个是调节水压，调节到最大的进行清洗
	//	}
		else
		{
		temp = WaterPressure[s1-1][s2];   //这个是调节水压
		}
	}
	else
	{
		...
		{
			temp = PengGang[s1-1][s2];		//这个是调节喷杆位置
		}
...
{	
	...
	Now_temp = Slet_position(t->select, p->Mode_select, p->Pres_select, p->Auto_clear);		//赋值当前状态的目标位置
	Dis_temp = Slet_position(t->select, p->Mode_select, p->Pres_select, p->Auto_clear) + WangFu_2mm;	//设定往复运动的目标位置
...
void Motor_work_prosess(Motor_uion **t, Status_uion *s)
{
	...
	//work step
	Index = s->Mode_select;
	Comsum = s->Pres_select;   //这里先把设定好的值赋值给变量，带后面再用
	switch(work.step)
	{
		case 0:
		...
		case 5:  //这里是切换功能时先切换到自洁口
			s->Auto_clear = 1;
			s->Pres_select = CskZJ;  //选择水压档位自洁档位
		...
		case 7:
			...
			/************以上是分水口初始化********************/
		case 8:
			s->Pres_select = Comsum;  //选择位置还原为设定值， Index 和 cumsum 这两个是 两个电机共用的
		...
		case 10: //这里要转到最大档位进行清洗2秒后再转到相应档位，防止档位过小，即热模块停止加热
			s->Auto_clear = 2;   //这里选择清洗
			s->Pres_select = 4;  //选择水压档位为最大
			Motor_set_position(p,s);    //这里是分水电机设置分水口位置
			Water.WaterIn_f = 1;
			Water.Hot_f = 1;
			work.step++;
		...
		case 11:   //等待分水器到位
			...
			if(p->end_f==1)
			{
				...
				s->Pres_select = Comsum;  //选择位置还原为设定值
				work.Start_f = 1;
		...
		case 15:   //喷杆位置微调和水压微调和往复运动操作步骤
			...
			{
				PenGan.PGmove_f = 0;
				s->Pres_select = s->PG_gear;   //喷杆档位赋值给选择档位，选择喷杆的位置
			...
			{
				Water.pressure_f = 0;
				s->Pres_select = s->Water_gear;   //水压档位赋值给选择档位，选择水压的位置
				Motor_set_position(p,s);    //这里是分水电机设置水压位置
			}
			//********往复运动********//
			if(s->reciprocate_f == 1)   
			{
				s->Pres_select = Comsum;
				s->Auto_clear = 0;		//清洗选择归0
			...
			if(work.time == 0) //清洗时间结束
			{
				...
				s->Pres_select = Comsum;  //选择目标位置还原为设定值，
				s->Mode_select = Index;  //选择位置还原为设定值，方便结束流程操作
...
void Motor_work_end(Motor_uion **t, Status_uion *s)
{
	...
	//-----------------------------等待伸缩杆到位			
		...
		case 6:
			s->Auto_clear = 1;
			s->Pres_select = CskMin;
			s->Mode_select += 50;
			Motor_set_position(p,s);    //这里是分水电机设置

			s->Pres_select = SSRestIndex;
			s->Mode_select = Index;
			Motor_set_position((p+1),s);    //这里是喷杆电机设置
			work.step++;
		break;
...
void Spout_clean(Motor_uion **t, Status_uion *s)
{
	...
	if(penkou.enable_f == 1)
	{
		CoverSeat.delay = T5Min;
		if(s->penkou_f == 1)
		{
			if(LiquidDeal.Enable_f == 1)
			{
				switch (penkou.step)
					{
					case 0:
