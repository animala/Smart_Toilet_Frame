...
void Power_Switch_deal(void)
{
	...
	//开机状态下，关机 需执行完所有正在进行的功能函数后再关机
	 if(Status.Power_e == 1)
	{
		if((Status.power_f == 1) && (Status.PG_zero_f == 1) && (Status.Divid_water_f == 1))   
		{
			Status.power_f = 0;		//电源状态关闭
			Status.Power_e = 0;  	//电源使能开启		
			Status.Icon.power_disp = 1;  //显示电源图标		
		}
	}
