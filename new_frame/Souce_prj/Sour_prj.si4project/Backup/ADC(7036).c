...
void Seat_temp_deal(ADC_Config *p)
{
	...
	Seat_temp_display(Seat.temperature, p->Err);   //显示风温
	
	if((Status.PcbTest_f == 0)&&(p->Err == 0))    //不在PCB测试下
	{
		if((Status.jieneng_e == 0)&&(Seat.DangWei == 0))
		{
			//------根据环境温度设置节能温度比较值
	...
	{
		if(Seat.temperature <= Seat_gear_table[Status.Icon.seat_gear - 1])  //当前温度对比档位温度
		{
			SeatHeat_set; 	
		}
		else if(Seat.temperature >= Seat_gear_table[Status.Icon.seat_gear - 1] + 1)
		{
			SeatHeat_clr; 	
		}
		Seat.Heat_f = 1;
...
void Fan_Temp_Deal(ADC_Config *p)
{
	...
	Room_temp_display(Drying.temperature, p->Err);   //----显示风温
