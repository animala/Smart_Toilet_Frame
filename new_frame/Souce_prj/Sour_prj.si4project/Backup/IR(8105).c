...
void IR_Opration(void)
{
	...
	//--------先反序读取数据，
		...
		checksum = 0;
		for(i=0; i<(IR_DataSize-1); i++) //累加前5个字节
		{
		...
		if(checksum == IR_databuffer[IR_DataSize-1])  //判断第一帧数据的校验和是否相等
		{
			...
			if(IR.TFDelay == 0)
			{
				Juge_checksum = checksum;	   //先把第一次的数据的检验和赋值给判断检验和变量
			...
			{
				if(Juge_checksum == checksum)  //如果第二次和第一次的一样，证明是重发的
				{
					for(i=0; i<(IR_DataSize-1); i++)
					{
						User_databuffer[i] = IR_databuffer[i+1];  //直接舍弃起始码，只赋值功能码和数据码来使用
