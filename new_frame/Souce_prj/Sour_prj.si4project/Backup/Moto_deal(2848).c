...
#define	Reciprocate_add	100  //这个是往复运动的位置补偿
...
void Motor_set_position(Motor_uion *t, Status_uion *p)
{
	...
	t->SumCountSet = Slet_position(t->select, p->Work_index, p->Comsum_set, p->Auto_clear);
...
{	
	...
	Now_temp = Slet_position(t->select, p->Work_index, p->Comsum_set, p->Auto_clear);		//赋值当前状态的目标位置
	Dis_temp = Slet_position(t->select, p->Work_index, 5, p->Auto_clear);	            //设定往复运动的目标位置
...
{	
	...
	switch(work.step)
	{
		...
		case 15:   //喷杆位置微调和水压微调和往复运动操作步骤
			...
			{
				Water.pressure_f = 0;
				s->Comsum_set = s->Water_gear;   //水压档位赋值给选择档位，选择水压的位置
				Motor_set_position(p,s);    //这里是分水电机设置水压位置
			}
			//********往复运动********//
			if(s->reciprocate_f == 1)   
			{
				s->Comsum_set = s->Pres_select;
