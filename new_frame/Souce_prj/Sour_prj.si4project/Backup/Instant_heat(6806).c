...
// 结构体数组处理，包含了UART0 和 UART1 的初始化，
// 预留了 数据长度、数据缓存大小的处理，移植时减小工作量
uart_str Uart_CB[2] = {
	{						//串口0的结构体初始化
		UART0_CH,
		Data_size, 
...
修改记录：暂未定义
------------------------------------------------*/
void Uart_Send_Data(uart_str *p)
{
	uchar i;
	for(i=0; i<p->len; i++)
	{
		Uart_TX_Byte(p->ch, (p->Tx_buff[i]));
	}
}
...
修改记录：
------------------------------------------------*/
void Uart_frame_send(uart_str *p)
{
	uchar	i;
	for(i=0; i<p->len; i++)//加载发送数据
	{
		p->Tx_buff[i] = p->Tx_data[i];
	}
	Uart_Send_Data(p);	
}
