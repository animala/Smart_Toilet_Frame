...
UART0信息定义
**************************************/ 	
uchar	rx0_buf[Data_size] = 0;		//----------UART0接收缓存器
uchar	rx0_data[Data_size] = 0;		//----------UART0接收数据
uchar	tx0_buf[Data_size] = 0;		//--------	UART0发送缓存器
uchar	tx0_data[Data_size] = 0;		//--------	UART0发送数据
...
修改记录：暂未定义
------------------------------------------------*/
void	Uart_TX_Byte(uint_8 ch,      uchar buf)
{
	if(ch == 0)
	{
...
修改记录：暂未定义
------------------------------------------------*/
void Uart_Send_Data(uchar uart_ch, uchar *buf,uchar len)
{
	uchar i;
	for(i=0; i<len; i++)
	{
		Uart_TX_Byte(uart_ch, *(buf+i));
	}
}
...
/*----------------------------------------------
函数名称：Uart_frame_send
功能描述：串行口发送一帧数据
输入参数：uart_ch 串口号
返回参数：无
修改记录：
------------------------------------------------*/
void Uart_frame_send(uchar uart_ch, uchar len)
{
	uchar	i;
	for(i=0; i<len; i++)//加载发送数据
	{
		tx0_buf[i] = tx0_data[i];
	}

	Uart_Send_Data(uart_ch, tx0_buf, len);	
}
...
{		
    ...
    if((uart_flag_f&0x01)==0x01)//UART0数据接收完成
	{
		...
		if(rx0_data[0]==0xcc)
		{
			...
			if(checksum==rx0_data[Data_size-1])//校验和判断
				uart_data_f|=0x01;
			else
				uart_data_f&=0xfe;	
		}
