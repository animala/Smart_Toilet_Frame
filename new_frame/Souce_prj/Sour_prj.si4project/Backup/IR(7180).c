	...
	//--------先反序读取数据，
		...
		if(IR.OK_f == 1)
		{
			...
			if(Status.power_f == 1)
			{
				...
				if((Func_select&0x19) == 0x19) //如果是参数调节
				{
					/*****以下是温度档位处理*****/			
					f = Gear_deal(User_databuffer[1]>>4);    //这个是水温
					Water_Gear_display(f);
					//Icon.set_gear = Gear_deal(User_databuffer[2]);    //这个是座温
					//Icon.wind_gear = Gear_deal(User_databuffer[3]);    //这个是风温
				...
				else if((Func_select&0x18) == 0x18) //如果是清洗调节
				{
					...
					//Icon.water_yaild = Gear_deal(User_databuffer[3]);    //这个是水量
				}
				else	//如果只是功能选择
				{
