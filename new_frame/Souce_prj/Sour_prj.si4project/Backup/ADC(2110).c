...
void New_temp_deal(ADC_Config *p)
{
	static uint_8 AdcCount = 0; 
	static uint_16 AdcBuf[5];
	uint_16 AdcBufMax;
...
void Seat_temp_deal(ADC_Config *p)
{
	...
	Display_Temp(4, Seat.temperature, 1);   //显示座温温度
...
{
	uint_16 ADtemp;
	//ADtemp = Temp_deal_fun(p, Wind_temp_channel);  //取出处理好的风温温度AD值		
	ADtemp = temp_save[1];
	...
	else
	{
		...
		Display_Temp(3, Drying.temperature, 0);   //显示座温温度
	}	
}
