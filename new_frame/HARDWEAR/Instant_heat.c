...
//---定义一个水温温度设定表格,本项目只有4档
const uint_8 Water_temp_table[4] = {
	//	32,	
		34,
		36,
		38,
...
void Uart0_Reverive(uart_str *p)
{
	if((SC0STR&0x10) == 0x10)
	{
		...
		if((p->Rx_flag & 0x01) == 0)
		{
			...
			p->Data_count++;
			if(p->Data_count > p->len)
			{
				p->Data_count = 0;
				p->Rx_flag |= 0x01;
			}
				
			/*p->Data_count = (p->Data_count+1) % p->len;	 //自加		
			if(p->Data_count == 0)
			{
				p->Rx_flag |= 0x01;
			}*/
...
{		
    ...
    if((p->Rx_flag & 0x01) == 0x01)   //UART0数据接收完成
	{
		...
		//------------------------判断接收数据是否正确
		if((p->Rx_data[0]==0x32) && (p->Rx_data[p->len-1]==0x34))  
		{
			checksum = 0;
			for(i=1;i<11;i++)
			{
			...
			if(checksum == p->Rx_data[Data_size-2]) //校验和判断
				p->Rx_flag |= 0x01;
			else
				p->Rx_flag &= 0xfe;	
		}
...
//
//********************************
void Tx_frame_buff(uart_str *p, uint_16 temp)
{
	uint_8 i;
	...
	p->Tx_data[4] = (temp*10) >> 8;		//Tset_H
	p->Tx_data[5] = (temp*10) & 0xff;		//Tset_L
	p->Tx_data[6] = 0x00;	
	p->Tx_data[7] = 0x00;	
	p->Tx_data[8] = 0x00;	
	p->Tx_data[9] = 0x00;	
	p->Tx_data[10] = 0x00;
	p->Tx_data[11] = 0;    //使用之前要清0
	for(i=1;i<11;i++)
	{
...
void Water_Temp_Deal(uart_str *p)
{
	...
	if(Status.power_f == 1)  //
	{
		...
		if(Water.TempDeal_time == 0) //如果接收下一帧的间隔时间到
		{
			if(Water.WaterIn_f == 1) //使能了进水
			{
				if(Water.Hot_f == 1)  //使能了加热
				{
					...
					else
					{
						...
						else if(Status.LengReAnMo_f == 1)	//冷热按摩
						{
							Water.delay1++;
							if((Water.delay1 > 0) && (Water.delay1 <= 10))	   //5秒加热   (500ms 进一次这里)
							{
								water_temp = 40;  
							}
							if((Water.delay1 > 10) && (Water.delay1 <= 12))   //1秒不加热 
							{
								water_temp = 0;  
							}
							if(Water.delay1 > 12)
						...
						{
							water_temp = Water_temp_table[Status.Icon.water_temp_gear];  //否则水温设定直接按照档位给温度
						}
					}
		...
		else
		{
			...
			if((p->Rx_flag & 0x01) == 0x01)  //如果接收到一帧数据了
			{
				p->Rx_flag &= ~0x01;	
				
				if(Water.JiRe_In_f == 1)
				{
					if(p->Rx_data[4] == 0)  //发过来故障码 没有故障   
					{
						Water.Error_flag &= 0xE3;	
						Water.InputTemp = p->Rx_data[5]*256 + p->Rx_data[6];   //进水温度
						Water.OutputTemp = (p->Rx_data[7]*256 + p->Rx_data[8])/10;		//出水温度
						Water.flow_count = p->Rx_data[9]*256 + p->Rx_data[10];		//进水流量
					} 
					//*******************进水温度故障处理****************************
					if(((p->Rx_data[4] && 0x02) == 0x02) || ((p->Rx_data[4] && 0x04) == 0x04)) //进水温传故障（开路/短路）
					{
					...
					//*******************出水温度故障处理****************************
					if(((p->Rx_data[4] && 0x08) == 0x08) || ((p->Rx_data[4] && 0x10) == 0x10)) //出水温传故障（开路/短路）
					{
					...
					//*******************温度过高故障处理****************************
					if((p->Rx_data[4] && 0x20) == 0x20)  //温度过高
					{
						Water.Error_flag |= 0x08;	
					}
					else
					{
						Water.Error_flag &= ~0x08;
				...
				{
					if(p->Rx_data[4] == 0)//通讯正常
					{
						Water.Error_flag &= 0xfe;	
						Water.InputTemp=p->Rx_data[5]*256 + p->Rx_data[6];   //进水温度
						Water.OutputTemp = (p->Rx_data[7]*256 + p->Rx_data[8])/10;		//出水温度
						Water.flow_count = p->Rx_data[9]*256 + p->Rx_data[10];		//进水流量
					}
